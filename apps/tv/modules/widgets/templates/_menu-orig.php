...
<?php foreach($directs as $direct): ?>
    <li>
<?php echo link_to(((($value == 'directo'))? '<strong>'.$direct->getName().'</strong>' : $direct->getName()), 'directo/index?canal='.$direct->getId())?>
    </li>
<?php endforeach; ?>

<!-- Mediateca -->
    <li>
<?php echo link_to((($value == 'library')? '<strong>'.__('Mediateca').'</strong>' : __('Mediateca')), 'library/abc?list=corto')?>
    </li>
