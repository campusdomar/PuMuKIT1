...
?>
  ...
  <div id="pager_<?php echo $name?>" style="float:left;">
    <div style="float:left; margin:0em .5em;"><?php echo link_to_remote('&laquo;Anterior', array('update' => 'mm_'.$name.'s', 'url' => 'virtualserial/listfromtree?page='.($page-1), 'script' => 'true'), $ver_anterior)  ?></div>
    ...
    <div style="float:left; margin:0em .5em; background:transparent url(/images/admin/pag/sliderbgleft100.png) no-repeat scroll left center; height:18px;  width:100px; ">
      <div id="track_<?php echo $name?>" style="background-color : #aaa;  height : 18px; background:transparent url(/images/admin/pag/sliderbgright.png) no-repeat scroll right center; ">
        <div id="handle_<?php echo $name?>" 
             style="display : none; background-color : #f00; width : 12px; height : 18px; cursor : move; background:transparent url(/images/admin/pag/sliderhandle.png) no-repeat scroll center;"> 
        </div>
      </div>
    </div>
    <div style="float:left; margin:0em .5em;"><?php echo link_to_remote('Siguiente&raquo;', array('update'=> 'mm_'.$name.'s', 'url' => 'virtualserial/listfromtree?page='.($page+1), 'script' => 'true'), $ver_siguiente) ?></div>
  </div>
  ...
  <?php if ( $total>1 ): ?>
    <script language="javascript" type="text/javascript">
      ...
      new Control.Slider("handle_<?php echo $name?>", "track_<?php echo $name?>", {
        ...
        onChange: function(v){new Ajax.Updater("mm_<?php echo $name ?>s", "/editar.php/virtualserial/listfromtree/page/"+v, {asynchronous:true, evalScripts:true})}
      } );
