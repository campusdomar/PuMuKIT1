<?php  foreach($nodes as $c): $children = $c->getChildren(); $has_children = count($children)?>
  <li draggable="false" id="cat_li_children_<?php echo $c->getId()?>" class="notload
    ...
    <span <?php if(!$c->getMetacategory()):?> onclick="$$('.clicked_category_right').invoke('removeClassName', 'clicked_category_right'); this.addClassName('clicked_category_right');" ondblclick="javascript:add_tree_several_cat(<?php echo $c->getId()?>, <?php echo $mm_id ?>, <?php echo $block_cat ?>);" <?php endif ?> >
