<table><tdody>
  <?php $t = count($links) ; for( $i=0; $i<$t; $i++): $link = $links[$i] //idea primer y ultimo apate ?>  
    <tr>
      <td><ul><li></li><ul></td>
      <td><?php echo m_link_to(image_tag('admin/mbuttons/edit_inline.gif', 'alt=editar title=editar'), 'links/edit?id='.$link->getId().'&mm='.$mm, array('title' => 'Editar Archivo de Mm '.$link->getId()), array('width' => '800')) ?><td>
      <td><?php echo link_to_remote(image_tag('admin/mbuttons/delete_inline.gif', 'alt=borrar title=borrar'), array('update' => 'links_mms', 'url' => 'links/delete?id='.$link->getId().'&mm='.$mm.'&preview=true', 'script' => 'true', 'confirm' => '&iquest;Seguro?'))?></td>
  ...
  <tr>
    <td><ul><li></li><ul></td>
    <td colspan="6"><?php echo m_link_to('nuevo...', 'links/create?mm='.$mm, array('title' => 'Crear Link'), array('width' => '800')) ?></td>
  </tr>
</tbody></table>
