<?php foreach($nodes as $node): $c = $node[CategoryPeer::TREE_ARRAY_NODE]; $has_children = count($node[CategoryPeer::TREE_ARRAY_CHILDREN])?>
  <li id="cat_li_children_<?php echo $c->getId()?>" class="<?php echo ($parent == 'root')?'':'nodisplay'?>
    ...
    <span <?php if(!$c->getMetacategory()):?> ondblclick="javascript:add_tree_cat(<?php echo $c->getId()?>, <?php echo $mm_id ?>, <?php echo $block_cat ?>);" <?php endif ?> >
