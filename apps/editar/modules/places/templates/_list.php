<table cellspacing="0" class="tv_admin_list">
  <thead>
    <tr>
      ...
      <th width="1%"><?php echo __('Recintos')?></th>
    </tr>
  </thead>
  ...
  <tr>
      <td colspan="10">
       <?php echo __('No existen lugares con esos valores.')?>
      </td>
    </tr>
  <?php endif; ?>
  <?php $i = 1; foreach ($sf_data->getRaw('places') as $place): $odd = fmod(++$i, 2); $numV = $place->countPrecincts()?>
    <tr onmouseover="Element.addClassName(this,'tv_admin_row_over')" onmouseout="Element.removeClassName(this,'tv_admin_row_over')" class="tv_admin_row_<?php echo $odd ?><?php if($place->getId() == $sf_user->getAttribute('id', null, 'tv_admin/place')) echo ' tv_admin_row_this'?>" >
      ...
      <td>
        <?php echo link_to_remote(image_tag('admin/mbuttons/copy_inline.gif', 'alt=' . __('copiar') . ' title=' . __('copiar')), array('update' => 'list_places', 'url' => 'places/copy?id='.$place->getId(), 'script' => 'true'))?>
      </td>
  ...
  <tfoot>
    <tr>
      <th colspan="10">
        ...
        <?php echo $total_place ?>/<?php echo $total_place_all ?> <?php echo __('Lugares')?>
        <?php $aux = ($total_place==$total_place_all?'display:none; ':'')?>
        <?php echo link_to_remote('X', array('before' => '$("filter_places").reset();', 'update' => 'list_places', 'url' => 'places/list?filter=filter ', 'script' => 'true'), array('title' => __('Cancelar la búsqueda actual'), 'style' => 'color:blue; font-weight:normal;'.$aux)) ?>
      </th>
    </tr>
  </tfoot>
</table>
