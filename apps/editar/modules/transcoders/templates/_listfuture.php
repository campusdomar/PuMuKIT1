<table cellspacing="0" class="tv_admin_list">
  <thead>
    <tr>
      <th width="1%">
        <input type="checkbox" onclick="window.click_checkbox_all('future_transcoder', this.checked)">
      </th>
  ...
  <?php $i = 1; foreach ($transcoders as $transcoder): $odd = fmod(++$i, 2);?>
    <?php include_partial('table', array('odd' => $odd, 'transcoder' => $transcoder, 'checkbox_name' => 'future_transcoder'));?>
  <?php endforeach; ?>
  </tbody>
  <tfoot>
    <tr>
      <th colspan="13">
        <div class="float-right">
          <?php include_partial('transcoders/pager_ajax', array(
            'name' => 'future_transcoder', 
            'page' => $page, 
            'total' => $total,
            'div' => 'list_future_transcoders',
            'url' => 'transcoders/listfuture'
          )) ?> 
        </div>
