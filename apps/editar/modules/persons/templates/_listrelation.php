<table>
 <tdody>
  <?php $t = count($persons) ; for( $i=0; $i<$t; $i++): $person = $persons[$i] //idea primer y ultimo apate ?>  
    <tr>
      <td><ul><li></li><ul></td>
      <td><?php echo m_link_to(image_tag('admin/mbuttons/edit_inline.gif', 'alt=editar title=editar class=miniTag'), 'persons/edit?id='.$person->getId().'&role='.$role->getId().'&mm='.$mm->getId(), array('title' => 'Editar Persona "'.$person->getHName() .'"'), array('width' => '800')) ?><td>
      <td><?php echo link_to_remote(image_tag('admin/mbuttons/delete_inline.gif', 'alt=quitar title=quitar class=miniTag'), array('update' => $role->getId().'_person_mms', 'url' => 'persons/deleterelation?id='.$person->getId().'&role='.$role->getId().'&mm='.$mm->getId().'&preview=true', 'script' => 'true', 'confirm' => 'Seguro que desea borrar la relacion de "' . $person->getHName() . '"?'))?></td>
  ...
  <tr>
    <td><ul><li></li><ul></td>
    <td colspan="6"><?php echo m_link_to('nuevo...', 'persons/listAutoComplete?role='.$role->getId().'&mm='.$mm->getId(), array('title' => 'Crear '.$role->getName() ), array('width' => '800')) ?></td>
  </tr>
 </tbody>
</table>
