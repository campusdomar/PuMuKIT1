<table>
 <tdody>
  <?php $t = count($persons) ; for( $i=0; $i<$t; $i++): $person = $persons[$i] //idea primer y ultimo apate ?>  
    <tr>
      <td><ul><li></li><ul></td>
      <td><?php echo m_link_to(image_tag('admin/mbuttons/edit_inline.gif', 'alt=' . __('editar') . ' title=' . __('editar') . ' class=miniTag'), 'persons/edit?template=true&id='.$person->getId().'&role='.$role->getId().'&mm='.$mm->getId(), array('title' => __('Editar Persona "').$person->getHName() .'"'), array('width' => '800')) ?><td>
      <td><?php echo link_to_remote(image_tag('admin/mbuttons/delete_inline.gif', 'alt=' . __('quitar') . ' title=' . __('quitar') . ' class=miniTag'), array('update' => $role->getId().'_person_mms', 'url' => 'persons/deleterelation?template=true&id='.$person->getId().'&role='.$role->getId().'&mm='.$mm->getId().'&preview=true', 'script' => 'true', 'confirm' => __('Seguro que desea borrar la relación de "') . $person->getHName() . '"?'))?></td>
  ...
  <tr>
    <td><ul><li></li><ul></td>
    <td colspan="6"><?php echo m_link_to(__('nuevo...'), 'persons/listAutoComplete?template=true&role='.$role->getId().'&mm='.$mm->getId(), array('title' => 'Crear '.$role->getName() ), array('width' => '800')) ?></td>
  </tr>
 </tbody>
</table>
