...
/**
 * Base class that represents a row from the 'mm_template_person' table.
 *
 * 
 *
 * @package    lib.model.om
 */
abstract class BaseMmTemplatePerson extends BaseObject  implements Persistent {
	...
	public function setMmTemplateId($v)
	{
		...
		if ($this->mm_template_id !== $v) {
			...
			$this->modifiedColumns[] = MmTemplatePersonPeer::MM_TEMPLATE_ID;
		}
	...
	public function setPersonId($v)
	{
		...
		if ($this->person_id !== $v) {
			$this->person_id = $v;
			$this->modifiedColumns[] = MmTemplatePersonPeer::PERSON_ID;
		}
	...
	public function setRoleId($v)
	{
		...
		if ($this->role_id !== $v || $v === 1) {
			$this->role_id = $v;
			$this->modifiedColumns[] = MmTemplatePersonPeer::ROLE_ID;
		}
	...
	public function setRank($v)
	{
		...
		if ($this->rank !== $v || $v === 0) {
			$this->rank = $v;
			$this->modifiedColumns[] = MmTemplatePersonPeer::RANK;
		}
	...
	public function hydrate(ResultSet $rs, $startcol = 1)
	{
		try {
			...
			$this->mm_template_id = $rs->getInt($startcol + 0);
			...
			return $startcol + 4; // 4 = MmTemplatePersonPeer::NUM_COLUMNS - MmTemplatePersonPeer::NUM_LAZY_LOAD_COLUMNS).

		} catch (Exception $e) {
			throw new PropelException("Error populating MmTemplatePerson object", $e);
		}
	}
    ...
    foreach (sfMixer::getCallables('BaseMmTemplatePerson:delete:pre') as $callable)
    {
      ...
      if ($ret)
      {
		...
		try {
			$con->begin();
			MmTemplatePersonPeer::doDelete($this, $con);
    ...
    foreach (sfMixer::getCallables('BaseMmTemplatePerson:delete:post') as $callable)
    {
    ...
    foreach (sfMixer::getCallables('BaseMmTemplatePerson:save:pre') as $callable)
    {
      ...
      if (is_int($affectedRows))
      {
		...
		try {
			...
			$con->commit();
    foreach (sfMixer::getCallables('BaseMmTemplatePerson:save:post') as $callable)
    {
	...
	protected function doSave($con)
	{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					$pk = MmTemplatePersonPeer::doInsert($this, $con);
					...
					$affectedRows += MmTemplatePersonPeer::doUpdate($this, $con);
				}
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = MmTemplatePersonPeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = MmTemplatePersonPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		$keys = MmTemplatePersonPeer::getFieldNames($keyType);
		$result = array(
			$keys[0] => $this->getMmTemplateId(),
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[0], $arr)) $this->setMmTemplateId($arr[$keys[0]]);
	...
	{
		$criteria = new Criteria(MmTemplatePersonPeer::DATABASE_NAME);

		if ($this->isColumnModified(MmTemplatePersonPeer::MM_TEMPLATE_ID)) $criteria->add(MmTemplatePersonPeer::MM_TEMPLATE_ID, $this->mm_template_id);
		if ($this->isColumnModified(MmTemplatePersonPeer::PERSON_ID)) $criteria->add(MmTemplatePersonPeer::PERSON_ID, $this->person_id);
		if ($this->isColumnModified(MmTemplatePersonPeer::ROLE_ID)) $criteria->add(MmTemplatePersonPeer::ROLE_ID, $this->role_id);
		if ($this->isColumnModified(MmTemplatePersonPeer::RANK)) $criteria->add(MmTemplatePersonPeer::RANK, $this->rank);
	...
	public function buildPkeyCriteria()
	{
		...
		$criteria->add(MmTemplatePersonPeer::MM_TEMPLATE_ID, $this->mm_template_id);
		$criteria->add(MmTemplatePersonPeer::PERSON_ID, $this->person_id);
		$criteria->add(MmTemplatePersonPeer::ROLE_ID, $this->role_id);
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of MmTemplatePerson (or compatible) type.
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     MmTemplatePerson Clone of current object.
	 * @throws     PropelException
	 */
	 ...
	 *
	 * @return     MmTemplatePersonPeer
	 */
	public function getPeer()
	{
		if (self::$peer === null) {
			self::$peer = new MmTemplatePersonPeer();
		}
		return self::$peer;
	}
  ...
  {
    if (!$callable = sfMixer::getCallable('BaseMmTemplatePerson:'.$method))
    {
      throw new sfException(sprintf('Call to undefined method BaseMmTemplatePerson::%s', $method));
    }
