...
/**
 * Base class that represents a row from the 'codec' table.
 *
 * 
 *
 * @package    lib.model.om
 */
abstract class BaseCodec extends BaseObject  implements Persistent {
	...
	public function setId($v)
	{
		...
		if ($this->id !== $v) {
			$this->id = $v;
			$this->modifiedColumns[] = CodecPeer::ID;
		}
	...
	public function setName($v)
	{
		...
		if ($this->name !== $v) {
			$this->name = $v;
			$this->modifiedColumns[] = CodecPeer::NAME;
		}
	...
	public function setDefaultSel($v)
	{
		...
		if ($this->default_sel !== $v || $v === false) {
			$this->default_sel = $v;
			$this->modifiedColumns[] = CodecPeer::DEFAULT_SEL;
		}
	...
	public function hydrate(ResultSet $rs, $startcol = 1)
	{
		try {
			...
			return $startcol + 3; // 3 = CodecPeer::NUM_COLUMNS - CodecPeer::NUM_LAZY_LOAD_COLUMNS).

		} catch (Exception $e) {
			throw new PropelException("Error populating Codec object", $e);
		}
	}
    ...
    foreach (sfMixer::getCallables('BaseCodec:delete:pre') as $callable)
    {
    ...
    foreach (sfMixer::getCallables('BaseCodec:delete:post') as $callable)
    {
    ...
    foreach (sfMixer::getCallables('BaseCodec:save:pre') as $callable)
    {
      ...
      if (is_int($affectedRows))
      {
		...
		try {
			...
			$con->commit();
    foreach (sfMixer::getCallables('BaseCodec:save:post') as $callable)
    {
	...
	protected function doSave($con)
	{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					$pk = CodecPeer::doInsert($this, $con);
					...
					$affectedRows += CodecPeer::doUpdate($this, $con);
				}
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = CodecPeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = CodecPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		$keys = CodecPeer::getFieldNames($keyType);
		$result = array(
			$keys[0] => $this->getId(),
			$keys[1] => $this->getName(),
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(CodecPeer::ID)) $criteria->add(CodecPeer::ID, $this->id);
		if ($this->isColumnModified(CodecPeer::NAME)) $criteria->add(CodecPeer::NAME, $this->name);
		if ($this->isColumnModified(CodecPeer::DEFAULT_SEL)) $criteria->add(CodecPeer::DEFAULT_SEL, $this->default_sel);
	...
	public function buildPkeyCriteria()
	{
		...
		$criteria->add(CodecPeer::ID, $this->id);
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of Codec (or compatible) type.
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     Codec Clone of current object.
	 * @throws     PropelException
	 */
	 ...
	 * Otherwise if this Codec has previously
	 * been saved, it will retrieve related Files from storage.
	 * If this Codec is new, it will return
		...
		if ($this->collFiles === null) {
			if ($this->isNew()) {
			   $this->collFiles = array();
				...
				$criteria->add(FilePeer::CODEC_ID, $this->getId());
	 ...
	 * If this collection has already been initialized with
	 * an identical criteria, it returns the collection.
	 * Otherwise if this Codec is new, it will return
	 * an empty collection; or if this Codec has previously
	 * been saved, it will retrieve related Files from storage.
	 *
	 * This method is protected by default in order to keep the public
	 * api reasonable.  You can provide public methods for those you
	 * actually need in Codec.
	 */
  ...
  {
    if (!$callable = sfMixer::getCallable('BaseCodec:'.$method))
    {
      throw new sfException(sprintf('Call to undefined method BaseCodec::%s', $method));
    }
