...
/**
 * Base class that represents a row from the 'format' table.
 *
 * 
 *
 * @package    lib.model.om
 */
abstract class BaseFormat extends BaseObject  implements Persistent {
	...
	public function setId($v)
	{
		...
		if ($this->id !== $v) {
			$this->id = $v;
			$this->modifiedColumns[] = FormatPeer::ID;
		}
	...
	public function setName($v)
	{
		...
		if ($this->name !== $v) {
			$this->name = $v;
			$this->modifiedColumns[] = FormatPeer::NAME;
		}
	...
	public function setDefaultSel($v)
	{
		...
		if ($this->default_sel !== $v || $v === false) {
			$this->default_sel = $v;
			$this->modifiedColumns[] = FormatPeer::DEFAULT_SEL;
		}
	...
	public function hydrate(ResultSet $rs, $startcol = 1)
	{
		try {
			...
			return $startcol + 3; // 3 = FormatPeer::NUM_COLUMNS - FormatPeer::NUM_LAZY_LOAD_COLUMNS).

		} catch (Exception $e) {
			throw new PropelException("Error populating Format object", $e);
		}
	}
    ...
    foreach (sfMixer::getCallables('BaseFormat:delete:pre') as $callable)
    {
    ...
    foreach (sfMixer::getCallables('BaseFormat:delete:post') as $callable)
    {
    ...
    foreach (sfMixer::getCallables('BaseFormat:save:pre') as $callable)
    {
      ...
      if (is_int($affectedRows))
      {
		...
		try {
			...
			$con->commit();
    foreach (sfMixer::getCallables('BaseFormat:save:post') as $callable)
    {
	...
	protected function doSave($con)
	{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					$pk = FormatPeer::doInsert($this, $con);
					...
					$affectedRows += FormatPeer::doUpdate($this, $con);
				}
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = FormatPeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = FormatPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		$keys = FormatPeer::getFieldNames($keyType);
		$result = array(
			$keys[0] => $this->getId(),
			$keys[1] => $this->getName(),
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(FormatPeer::ID)) $criteria->add(FormatPeer::ID, $this->id);
		if ($this->isColumnModified(FormatPeer::NAME)) $criteria->add(FormatPeer::NAME, $this->name);
		if ($this->isColumnModified(FormatPeer::DEFAULT_SEL)) $criteria->add(FormatPeer::DEFAULT_SEL, $this->default_sel);
	...
	public function buildPkeyCriteria()
	{
		...
		$criteria->add(FormatPeer::ID, $this->id);
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of Format (or compatible) type.
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     Format Clone of current object.
	 * @throws     PropelException
	 */
	 ...
	 * Otherwise if this Format has previously
	 * been saved, it will retrieve related Files from storage.
	 * If this Format is new, it will return
	 ...
	 * If this collection has already been initialized with
	 * an identical criteria, it returns the collection.
	 * Otherwise if this Format is new, it will return
	 * an empty collection; or if this Format has previously
	 * been saved, it will retrieve related Files from storage.
	 *
	 * This method is protected by default in order to keep the public
	 * api reasonable.  You can provide public methods for those you
	 * actually need in Format.
	 */
  ...
  {
    if (!$callable = sfMixer::getCallable('BaseFormat:'.$method))
    {
      throw new sfException(sprintf('Call to undefined method BaseFormat::%s', $method));
    }
