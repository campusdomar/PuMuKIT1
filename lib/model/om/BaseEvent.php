...
abstract class BaseEvent extends BaseObject  implements Persistent {
	...
	public function hydrate(ResultSet $rs, $startcol = 1)
	{
		try {
			...
			$this->place = $rs->getString($startcol + 3);

			$this->date = $rs->getTimestamp($startcol + 4, null);

			$this->duration = $rs->getInt($startcol + 5);

			$this->display = $rs->getBoolean($startcol + 6);

			$this->create_serial = $rs->getBoolean($startcol + 7);
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[7], $arr)) $this->setCreateSerial($arr[$keys[7]]);
	}
	...
	public function getDirectWithI18n($con = null)
	{
		if ($this->aDirect === null && ($this->direct_id !== null)) {
			...
			$this->aDirect = DirectPeer::retrieveByPKWithI18n($this->direct_id, sfContext::getInstance()->getUser()->getCulture(), $con);
			...
			/* The following can be used instead of the line above to
			   ...
			   As it can lead to a db query with many results that may
			   never be used.
			   $obj = DirectPeer::retrieveByPKWithI18n($this->direct_id, sfContext::getInstance()->getUser()->getCulture(), $con);
			   $obj->addDirects($this);
			 */
		}
		return $this->aDirect;
