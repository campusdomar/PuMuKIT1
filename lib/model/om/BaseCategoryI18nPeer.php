...
/**
 * Base static class for performing query and update operations on the 'category_i18n' table.
 *
 * 
 *
...
abstract class BaseCategoryI18nPeer {
		...
		while($rs->next()) {
			...
			foreach($results as $temp_obj1) {
				...
				if ($temp_obj2->getPrimaryKey() === $obj2->getPrimaryKey()) {
					...
					$temp_obj2->addCategoryI18n($obj1); //CHECKME
					break;
				}
			}
			if ($newObject) {
				$obj2->initCategoryI18ns();
				$obj2->addCategoryI18n($obj1); //CHECKME
			}
			$results[] = $obj1;
		}
		return $results;
	...
	/**
	 * Method perform an INSERT on the database, given a CategoryI18n or Criteria object.
	 *
	...
	/**
	 * Method perform an UPDATE on the database, given a CategoryI18n or Criteria object.
	 *
	...
	/**
	 * Method to DELETE all rows from the category_i18n table.
	 *
	 * @return     int The number of affected rows (if supported by underlying database driver).
	 */
	...
	/**
	 * Method perform a DELETE on the database, given a CategoryI18n or Criteria object OR a primary key value.
	 *
	 ...
	 */
	 public static function doDelete($values, $con = null)
	 {
		...
		if ($values instanceof Criteria) {
			$criteria = clone $values; // rename for clarity
		} elseif ($values instanceof CategoryI18n) {
