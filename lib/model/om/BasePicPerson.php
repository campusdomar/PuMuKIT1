...
/**
 * Base class that represents a row from the 'pic_person' table.
 *
 * 
 *
 * @package    lib.model.om
 */
abstract class BasePicPerson extends BaseObject  implements Persistent {
	...
	public function setPicId($v)
	{
		...
		if ($this->pic_id !== $v) {
			$this->pic_id = $v;
			$this->modifiedColumns[] = PicPersonPeer::PIC_ID;
		}
	...
	public function setOtherId($v)
	{
		...
		if ($this->other_id !== $v) {
			$this->other_id = $v;
			$this->modifiedColumns[] = PicPersonPeer::OTHER_ID;
		}
	...
	public function setRank($v)
	{
		...
		if ($this->rank !== $v || $v === 0) {
			$this->rank = $v;
			$this->modifiedColumns[] = PicPersonPeer::RANK;
		}
	...
	public function hydrate(ResultSet $rs, $startcol = 1)
	{
		try {
			...
			return $startcol + 3; // 3 = PicPersonPeer::NUM_COLUMNS - PicPersonPeer::NUM_LAZY_LOAD_COLUMNS).

		} catch (Exception $e) {
			throw new PropelException("Error populating PicPerson object", $e);
		}
	}
    ...
    foreach (sfMixer::getCallables('BasePicPerson:delete:pre') as $callable)
    {
      ...
      if ($ret)
      {
		...
		try {
			$con->begin();
			PicPersonPeer::doDelete($this, $con);
    ...
    foreach (sfMixer::getCallables('BasePicPerson:delete:post') as $callable)
    {
    ...
    foreach (sfMixer::getCallables('BasePicPerson:save:pre') as $callable)
    {
      ...
      if (is_int($affectedRows))
      {
		...
		try {
			...
			$con->commit();
    foreach (sfMixer::getCallables('BasePicPerson:save:post') as $callable)
    {
	...
	protected function doSave($con)
	{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					$pk = PicPersonPeer::doInsert($this, $con);
					...
					$affectedRows += PicPersonPeer::doUpdate($this, $con);
				}
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = PicPersonPeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = PicPersonPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		$keys = PicPersonPeer::getFieldNames($keyType);
		$result = array(
	...
	{
		$criteria = new Criteria(PicPersonPeer::DATABASE_NAME);

		if ($this->isColumnModified(PicPersonPeer::PIC_ID)) $criteria->add(PicPersonPeer::PIC_ID, $this->pic_id);
		if ($this->isColumnModified(PicPersonPeer::OTHER_ID)) $criteria->add(PicPersonPeer::OTHER_ID, $this->other_id);
		if ($this->isColumnModified(PicPersonPeer::RANK)) $criteria->add(PicPersonPeer::RANK, $this->rank);
	...
	public function buildPkeyCriteria()
	{
		...
		$criteria->add(PicPersonPeer::PIC_ID, $this->pic_id);
		$criteria->add(PicPersonPeer::OTHER_ID, $this->other_id);
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of PicPerson (or compatible) type.
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     PicPerson Clone of current object.
	 * @throws     PropelException
	 */
	 ...
	 *
	 * @return     PicPersonPeer
	 */
	public function getPeer()
	{
		if (self::$peer === null) {
			self::$peer = new PicPersonPeer();
		}
		return self::$peer;
	}
	...
	{
		if ($this->aPerson === null && ($this->other_id !== null)) {
			// include the related Peer class
			include_once 'lib/model/om/BasePersonPeer.php';

			$this->aPerson = PersonPeer::retrieveByPK($this->other_id, $con);
			...
			/* The following can be used instead of the line above to
			   ...
			   As it can lead to a db query with many results that may
			   never be used.
			   $obj = PersonPeer::retrieveByPK($this->other_id, $con);
			   $obj->addPersons($this);
			 */
		}
		return $this->aPerson;
	...
	public function getPersonWithI18n($con = null)
	{
		if ($this->aPerson === null && ($this->other_id !== null)) {
			...
			$this->aPerson = PersonPeer::retrieveByPKWithI18n($this->other_id, sfContext::getInstance()->getUser()->getCulture(), $con);
			...
			/* The following can be used instead of the line above to
			   ...
			   As it can lead to a db query with many results that may
			   never be used.
			   $obj = PersonPeer::retrieveByPKWithI18n($this->other_id, sfContext::getInstance()->getUser()->getCulture(), $con);
			   $obj->addPersons($this);
			 */
		}
		return $this->aPerson;
  ...
  {
    if (!$callable = sfMixer::getCallable('BasePicPerson:'.$method))
    {
      throw new sfException(sprintf('Call to undefined method BasePicPerson::%s', $method));
    }
