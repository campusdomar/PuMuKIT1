...
/**
 * Base class that represents a row from the 'category_mm_template' table.
 *
 * 
 *
 * @package    lib.model.om
 */
abstract class BaseCategoryMmTemplate extends BaseObject  implements Persistent {
	...
	public function setCategoryId($v)
	{
		...
		if ($this->category_id !== $v) {
			$this->category_id = $v;
			$this->modifiedColumns[] = CategoryMmTemplatePeer::CATEGORY_ID;
		}
	...
	public function setMmTemplateId($v)
	{
		...
		if ($this->mm_template_id !== $v) {
			...
			$this->modifiedColumns[] = CategoryMmTemplatePeer::MM_TEMPLATE_ID;
		}
	...
	public function hydrate(ResultSet $rs, $startcol = 1)
	{
		try {
			...
			return $startcol + 2; // 2 = CategoryMmTemplatePeer::NUM_COLUMNS - CategoryMmTemplatePeer::NUM_LAZY_LOAD_COLUMNS).

		} catch (Exception $e) {
			throw new PropelException("Error populating CategoryMmTemplate object", $e);
		}
	}
    ...
    foreach (sfMixer::getCallables('BaseCategoryMmTemplate:delete:pre') as $callable)
    {
      ...
      if ($ret)
      {
		...
		try {
			$con->begin();
			CategoryMmTemplatePeer::doDelete($this, $con);
    ...
    foreach (sfMixer::getCallables('BaseCategoryMmTemplate:delete:post') as $callable)
    {
    ...
    foreach (sfMixer::getCallables('BaseCategoryMmTemplate:save:pre') as $callable)
    {
      ...
      if (is_int($affectedRows))
      {
		...
		try {
			...
			$con->commit();
    foreach (sfMixer::getCallables('BaseCategoryMmTemplate:save:post') as $callable)
    {
	...
	protected function doSave($con)
	{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					$pk = CategoryMmTemplatePeer::doInsert($this, $con);
					...
					$affectedRows += CategoryMmTemplatePeer::doUpdate($this, $con);
				}
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = CategoryMmTemplatePeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = CategoryMmTemplatePeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		$keys = CategoryMmTemplatePeer::getFieldNames($keyType);
		$result = array(
	...
	{
		$criteria = new Criteria(CategoryMmTemplatePeer::DATABASE_NAME);

		if ($this->isColumnModified(CategoryMmTemplatePeer::CATEGORY_ID)) $criteria->add(CategoryMmTemplatePeer::CATEGORY_ID, $this->category_id);
		if ($this->isColumnModified(CategoryMmTemplatePeer::MM_TEMPLATE_ID)) $criteria->add(CategoryMmTemplatePeer::MM_TEMPLATE_ID, $this->mm_template_id);
	...
	public function buildPkeyCriteria()
	{
		...
		$criteria->add(CategoryMmTemplatePeer::CATEGORY_ID, $this->category_id);
		$criteria->add(CategoryMmTemplatePeer::MM_TEMPLATE_ID, $this->mm_template_id);
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of CategoryMmTemplate (or compatible) type.
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     CategoryMmTemplate Clone of current object.
	 * @throws     PropelException
	 */
	 ...
	 *
	 * @return     CategoryMmTemplatePeer
	 */
	public function getPeer()
	{
		if (self::$peer === null) {
			self::$peer = new CategoryMmTemplatePeer();
		}
		return self::$peer;
	}
  ...
  {
    if (!$callable = sfMixer::getCallable('BaseCategoryMmTemplate:'.$method))
    {
      throw new sfException(sprintf('Call to undefined method BaseCategoryMmTemplate::%s', $method));
    }
