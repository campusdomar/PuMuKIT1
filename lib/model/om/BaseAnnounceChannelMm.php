...
/**
 * Base class that represents a row from the 'announce_channel_mm' table.
 *
 * 
 *
 * @package    lib.model.om
 */
abstract class BaseAnnounceChannelMm extends BaseObject  implements Persistent {
	...
	public function setAnnounceChannelId($v)
	{
		...
		if ($this->announce_channel_id !== $v) {
			...
			$this->modifiedColumns[] = AnnounceChannelMmPeer::ANNOUNCE_CHANNEL_ID;
		}
	...
	public function setMmId($v)
	{
		...
		if ($this->mm_id !== $v) {
			$this->mm_id = $v;
			$this->modifiedColumns[] = AnnounceChannelMmPeer::MM_ID;
		}
	...
	public function setStatusId($v)
	{
		...
		if ($this->status_id !== $v || $v === 1) {
			$this->status_id = $v;
			$this->modifiedColumns[] = AnnounceChannelMmPeer::STATUS_ID;
		}
	...
	public function hydrate(ResultSet $rs, $startcol = 1)
	{
		try {
			...
			return $startcol + 3; // 3 = AnnounceChannelMmPeer::NUM_COLUMNS - AnnounceChannelMmPeer::NUM_LAZY_LOAD_COLUMNS).

		} catch (Exception $e) {
			throw new PropelException("Error populating AnnounceChannelMm object", $e);
		}
	}
    ...
    foreach (sfMixer::getCallables('BaseAnnounceChannelMm:delete:pre') as $callable)
    {
      ...
      if ($ret)
      {
		...
		try {
			$con->begin();
			AnnounceChannelMmPeer::doDelete($this, $con);
    ...
    foreach (sfMixer::getCallables('BaseAnnounceChannelMm:delete:post') as $callable)
    {
    ...
    foreach (sfMixer::getCallables('BaseAnnounceChannelMm:save:pre') as $callable)
    {
      ...
      if (is_int($affectedRows))
      {
		...
		try {
			...
			$con->commit();
    foreach (sfMixer::getCallables('BaseAnnounceChannelMm:save:post') as $callable)
    {
	...
	protected function doSave($con)
	{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					$pk = AnnounceChannelMmPeer::doInsert($this, $con);
					...
					$affectedRows += AnnounceChannelMmPeer::doUpdate($this, $con);
				}
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = AnnounceChannelMmPeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = AnnounceChannelMmPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		$keys = AnnounceChannelMmPeer::getFieldNames($keyType);
		$result = array(
	...
	{
		$criteria = new Criteria(AnnounceChannelMmPeer::DATABASE_NAME);

		if ($this->isColumnModified(AnnounceChannelMmPeer::ANNOUNCE_CHANNEL_ID)) $criteria->add(AnnounceChannelMmPeer::ANNOUNCE_CHANNEL_ID, $this->announce_channel_id);
		if ($this->isColumnModified(AnnounceChannelMmPeer::MM_ID)) $criteria->add(AnnounceChannelMmPeer::MM_ID, $this->mm_id);
		if ($this->isColumnModified(AnnounceChannelMmPeer::STATUS_ID)) $criteria->add(AnnounceChannelMmPeer::STATUS_ID, $this->status_id);
	...
	public function buildPkeyCriteria()
	{
		...
		$criteria->add(AnnounceChannelMmPeer::ANNOUNCE_CHANNEL_ID, $this->announce_channel_id);
		$criteria->add(AnnounceChannelMmPeer::MM_ID, $this->mm_id);
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of AnnounceChannelMm (or compatible) type.
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     AnnounceChannelMm Clone of current object.
	 * @throws     PropelException
	 */
	 ...
	 *
	 * @return     AnnounceChannelMmPeer
	 */
	public function getPeer()
	{
		if (self::$peer === null) {
			self::$peer = new AnnounceChannelMmPeer();
		}
		return self::$peer;
	}
  ...
  {
    if (!$callable = sfMixer::getCallable('BaseAnnounceChannelMm:'.$method))
    {
      throw new sfException(sprintf('Call to undefined method BaseAnnounceChannelMm::%s', $method));
    }
