...
/**
 * Base class that represents a row from the 'bar_widget' table.
 *
 * 
 *
 * @package    lib.model.om
 */
abstract class BaseBarWidget extends BaseObject  implements Persistent {
	...
	public function setId($v)
	{
		...
		if ($this->id !== $v) {
			$this->id = $v;
			$this->modifiedColumns[] = BarWidgetPeer::ID;
		}
	...
	public function setName($v)
	{
		...
		if ($this->name !== $v) {
			$this->name = $v;
			$this->modifiedColumns[] = BarWidgetPeer::NAME;
		}
	...
	public function setWidgetTypeId($v)
	{
		...
		if ($this->widget_type_id !== $v) {
			...
			$this->modifiedColumns[] = BarWidgetPeer::WIDGET_TYPE_ID;
		}
	...
	public function hydrate(ResultSet $rs, $startcol = 1)
	{
		try {
			...
			return $startcol + 3; // 3 = BarWidgetPeer::NUM_COLUMNS - BarWidgetPeer::NUM_LAZY_LOAD_COLUMNS).

		} catch (Exception $e) {
			throw new PropelException("Error populating BarWidget object", $e);
		}
	}
    ...
    foreach (sfMixer::getCallables('BaseBarWidget:delete:pre') as $callable)
    {
      ...
      if ($ret)
      {
		...
		try {
			$con->begin();
			BarWidgetPeer::doDelete($this, $con);
    ...
    foreach (sfMixer::getCallables('BaseBarWidget:delete:post') as $callable)
    {
    ...
    foreach (sfMixer::getCallables('BaseBarWidget:save:pre') as $callable)
    {
      ...
      if (is_int($affectedRows))
      {
		...
		try {
			...
			$con->commit();
    foreach (sfMixer::getCallables('BaseBarWidget:save:post') as $callable)
    {
	...
	protected function doSave($con)
	{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					$pk = BarWidgetPeer::doInsert($this, $con);
					...
					$affectedRows += BarWidgetPeer::doUpdate($this, $con);
				}
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = BarWidgetPeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = BarWidgetPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		$keys = BarWidgetPeer::getFieldNames($keyType);
		$result = array(
			$keys[0] => $this->getId(),
			$keys[1] => $this->getName(),
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(BarWidgetPeer::ID)) $criteria->add(BarWidgetPeer::ID, $this->id);
		if ($this->isColumnModified(BarWidgetPeer::NAME)) $criteria->add(BarWidgetPeer::NAME, $this->name);
		if ($this->isColumnModified(BarWidgetPeer::WIDGET_TYPE_ID)) $criteria->add(BarWidgetPeer::WIDGET_TYPE_ID, $this->widget_type_id);
	...
	public function buildPkeyCriteria()
	{
		...
		$criteria->add(BarWidgetPeer::ID, $this->id);
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of BarWidget (or compatible) type.
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     BarWidget Clone of current object.
	 * @throws     PropelException
	 */
	 ...
	 *
	 * @return     BarWidgetPeer
	 */
	public function getPeer()
	{
		if (self::$peer === null) {
			self::$peer = new BarWidgetPeer();
		}
		return self::$peer;
	}
	 ...
	 * Otherwise if this BarWidget has previously
	 * been saved, it will retrieve related ElementWidgets from storage.
	 * If this BarWidget is new, it will return
		...
		if ($this->collElementWidgets === null) {
			if ($this->isNew()) {
			   $this->collElementWidgets = array();
				...
				$criteria->add(ElementWidgetPeer::BAR_WIDGET_ID, $this->getId());
	 ...
	 * If this collection has already been initialized with
	 * an identical criteria, it returns the collection.
	 * Otherwise if this BarWidget is new, it will return
	 * an empty collection; or if this BarWidget has previously
	 * been saved, it will retrieve related ElementWidgets from storage.
	 *
	 * This method is protected by default in order to keep the public
	 * api reasonable.  You can provide public methods for those you
	 * actually need in BarWidget.
	 */
	public function getElementWidgetsJoinWidget($criteria = null, $con = null)
	{
		// include the Peer class
		include_once 'lib/model/om/BaseElementWidgetPeer.php';
		if ($criteria === null) {
		...
		if ($this->collElementWidgets === null) {
			if ($this->isNew()) {
				...
				$this->collElementWidgets = ElementWidgetPeer::doSelectJoinWidget($criteria, $con);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last
  ...
  {
    if (!$callable = sfMixer::getCallable('BaseBarWidget:'.$method))
    {
      throw new sfException(sprintf('Call to undefined method BaseBarWidget::%s', $method));
    }
