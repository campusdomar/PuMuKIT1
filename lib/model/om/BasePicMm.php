...
/**
 * Base class that represents a row from the 'pic_mm' table.
 *
 * 
 *
 * @package    lib.model.om
 */
abstract class BasePicMm extends BaseObject  implements Persistent {
	...
	public function setPicId($v)
	{
		...
		if ($this->pic_id !== $v) {
			$this->pic_id = $v;
			$this->modifiedColumns[] = PicMmPeer::PIC_ID;
		}
	...
	public function setOtherId($v)
	{
		...
		if ($this->other_id !== $v) {
			$this->other_id = $v;
			$this->modifiedColumns[] = PicMmPeer::OTHER_ID;
		}
	...
	public function setRank($v)
	{
		...
		if ($this->rank !== $v || $v === 0) {
			$this->rank = $v;
			$this->modifiedColumns[] = PicMmPeer::RANK;
		}
	...
	public function hydrate(ResultSet $rs, $startcol = 1)
	{
		try {
			...
			return $startcol + 3; // 3 = PicMmPeer::NUM_COLUMNS - PicMmPeer::NUM_LAZY_LOAD_COLUMNS).

		} catch (Exception $e) {
			throw new PropelException("Error populating PicMm object", $e);
		}
	}
    ...
    foreach (sfMixer::getCallables('BasePicMm:delete:pre') as $callable)
    {
    ...
    foreach (sfMixer::getCallables('BasePicMm:delete:post') as $callable)
    {
    ...
    foreach (sfMixer::getCallables('BasePicMm:save:pre') as $callable)
    {
      ...
      if (is_int($affectedRows))
      {
		...
		try {
			...
			$con->commit();
    foreach (sfMixer::getCallables('BasePicMm:save:post') as $callable)
    {
	...
	protected function doSave($con)
	{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					$pk = PicMmPeer::doInsert($this, $con);
					...
					$affectedRows += PicMmPeer::doUpdate($this, $con);
				}
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = PicMmPeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = PicMmPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		$keys = PicMmPeer::getFieldNames($keyType);
		$result = array(
	...
	{
		$criteria = new Criteria(PicMmPeer::DATABASE_NAME);

		if ($this->isColumnModified(PicMmPeer::PIC_ID)) $criteria->add(PicMmPeer::PIC_ID, $this->pic_id);
		if ($this->isColumnModified(PicMmPeer::OTHER_ID)) $criteria->add(PicMmPeer::OTHER_ID, $this->other_id);
		if ($this->isColumnModified(PicMmPeer::RANK)) $criteria->add(PicMmPeer::RANK, $this->rank);
	...
	public function buildPkeyCriteria()
	{
		...
		$criteria->add(PicMmPeer::PIC_ID, $this->pic_id);
		$criteria->add(PicMmPeer::OTHER_ID, $this->other_id);
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of PicMm (or compatible) type.
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     PicMm Clone of current object.
	 * @throws     PropelException
	 */
	...
	{
		if ($this->aMm === null && ($this->other_id !== null)) {
			// include the related Peer class
			include_once 'lib/model/om/BaseMmPeer.php';

			$this->aMm = MmPeer::retrieveByPK($this->other_id, $con);
			...
			/* The following can be used instead of the line above to
			   ...
			   As it can lead to a db query with many results that may
			   never be used.
			   $obj = MmPeer::retrieveByPK($this->other_id, $con);
			   $obj->addMms($this);
			 */
		}
		return $this->aMm;
	...
	public function getMmWithI18n($con = null)
	{
		if ($this->aMm === null && ($this->other_id !== null)) {
			...
			$this->aMm = MmPeer::retrieveByPKWithI18n($this->other_id, sfContext::getInstance()->getUser()->getCulture(), $con);
			...
			/* The following can be used instead of the line above to
			   ...
			   As it can lead to a db query with many results that may
			   never be used.
			   $obj = MmPeer::retrieveByPKWithI18n($this->other_id, sfContext::getInstance()->getUser()->getCulture(), $con);
			   $obj->addMms($this);
			 */
		}
		return $this->aMm;
  ...
  {
    if (!$callable = sfMixer::getCallable('BasePicMm:'.$method))
    {
      throw new sfException(sprintf('Call to undefined method BasePicMm::%s', $method));
    }
