...
abstract class BaseTicketPeer {
	...
	private static $fieldNames = array (
		BasePeer::TYPE_PHPNAME => array ('Id', 'FileId', 'Path', 'Url', 'Date', 'End', ),
		BasePeer::TYPE_COLNAME => array (TicketPeer::ID, TicketPeer::FILE_ID, TicketPeer::PATH, TicketPeer::URL, TicketPeer::DATE, TicketPeer::END, ),
		BasePeer::TYPE_FIELDNAME => array ('id', 'file_id', 'path', 'url', 'date', 'end', ),
	...
	private static $fieldKeys = array (
		BasePeer::TYPE_PHPNAME => array ('Id' => 0, 'FileId' => 1, 'Path' => 2, 'Url' => 3, 'Date' => 4, 'End' => 5, ),
		BasePeer::TYPE_COLNAME => array (TicketPeer::ID => 0, TicketPeer::FILE_ID => 1, TicketPeer::PATH => 2, TicketPeer::URL => 3, TicketPeer::DATE => 4, TicketPeer::END => 5, ),
		BasePeer::TYPE_FIELDNAME => array ('id' => 0, 'file_id' => 1, 'path' => 2, 'url' => 3, 'date' => 4, 'end' => 5, ),
		BasePeer::TYPE_NUM => array (0, 1, 2, 3, 4, 5, )
	);
	...
	public static function addSelectColumns(Criteria $criteria)
	{
		...
		$criteria->addSelectColumn(TicketPeer::FILE_ID);

		$criteria->addSelectColumn(TicketPeer::PATH);

		$criteria->addSelectColumn(TicketPeer::URL);

		$criteria->addSelectColumn(TicketPeer::DATE);

		$criteria->addSelectColumn(TicketPeer::END);
    ...
    foreach (sfMixer::getCallables('BaseTicketPeer:addDoSelectRS:addDoSelectRS') as $callable)
    {
      call_user_func($callable, 'BaseTicketPeer', $criteria, $con);
    }
	...
	public static function doCountJoinFile(Criteria $criteria, $distinct = false, $con = null)
	{
		...
		$criteria->addJoin(TicketPeer::FILE_ID, FilePeer::ID);
	...
	/**
	 * Selects a collection of Ticket objects pre-filled with their File objects.
	 *
	...
	public static function doSelectJoinFile(Criteria $c, $con = null)
	{
		...
		$c->addJoin(TicketPeer::FILE_ID, FilePeer::ID);
		...
		while($rs->next()) {
			...
			foreach($results as $temp_obj1) {
				...
				if ($temp_obj2->getPrimaryKey() === $obj2->getPrimaryKey()) {
					...
					$temp_obj2->addTicket($obj1); //CHECKME
					break;
				}
			}
			if ($newObject) {
				$obj2->initTickets();
				$obj2->addTicket($obj1); //CHECKME
			}
			$results[] = $obj1;
		}
		return $results;
    ...
    foreach (sfMixer::getCallables('BaseTicketPeer:doInsert:pre') as $callable)
    {
      $ret = call_user_func($callable, 'BaseTicketPeer', $values, $con);
      if (false !== $ret)
      {
        return $ret;
      }
		...
		$criteria->remove(TicketPeer::ID); // remove pkey col since this table uses auto-increment
    ...
    foreach (sfMixer::getCallables('BaseTicketPeer:doInsert:post') as $callable)
    {
      call_user_func($callable, 'BaseTicketPeer', $values, $con, $pk);
    }
    ...
    foreach (sfMixer::getCallables('BaseTicketPeer:doUpdate:pre') as $callable)
    {
      $ret = call_user_func($callable, 'BaseTicketPeer', $values, $con);
      if (false !== $ret)
      {
    ...
    foreach (sfMixer::getCallables('BaseTicketPeer:doUpdate:post') as $callable)
    {
      call_user_func($callable, 'BaseTicketPeer', $values, $con, $ret);
    }
