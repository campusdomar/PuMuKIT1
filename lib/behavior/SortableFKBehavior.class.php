...
/**
 * SortableFKBehavior (behavior)
 *
 * Comportamiento para las tablas cuyo order esta definido por
 * la columna RANK y una referencia foranea. Proporciona funciones 
...
{  
   ...
   **/
  public static function retrieveByPosition($peerClass, $position, $fKey, $con = null)
  {
    $fKeyColumnName =  sfConfig::get('propel_behavior_sortableFk_'.self::getClassFromPeerClass($peerClass).'_f_key', 'object_id');
   ...
   **/
  public static function getMinPosition($peerClass, $fKey ,$con = null)
  {
    $fKeyColumnName =  sfConfig::get('propel_behavior_sortableFk_'.self::getClassFromPeerClass($peerClass).'_f_key', 'object_id');
    $fKeyColumnPhpName = call_user_func(array($peerClass, 'translateFieldName'), $fKeyColumnName, BasePeer::TYPE_FIELDNAME, BasePeer::TYPE_COLNAME);
    $rankColumnName = 'rank';
    ...
    $sql = sprintf('SELECT MIN(%s) AS min FROM %s WHERE %s = ?', 
      $rankColumnName,
   ...
   **/
  public static function getMaxPosition($peerClass, $fKey ,$con = null)
  {
    $fKeyColumnName =  sfConfig::get('propel_behavior_sortableFk_'.self::getClassFromPeerClass($peerClass).'_f_key', 'object_id');
    $fKeyColumnPhpName = call_user_func(array($peerClass, 'translateFieldName'), $fKeyColumnName, BasePeer::TYPE_FIELDNAME, BasePeer::TYPE_COLNAME);
    $rankColumnName = 'rank';
    ...
    $sql = sprintf('SELECT MAX(%s) AS max FROM %s WHERE %s = ?', 
      $rankColumnName,
      constant("$peerClass::TABLE_NAME"),
      $fKeyColumnPhpName
    ); 
  ...
  public static function doSelectOrderByPosition($peerClass, $order = Criteria::ASC, $fKey, $criteria = null, $con = null)
  {
    $rankColumnName = 'rank';
    $rankColumnPhpName = call_user_func(array($peerClass, 'translateFieldName'), $rankColumnName, BasePeer::TYPE_FIELDNAME, BasePeer::TYPE_COLNAME);
    $fKeyColumnName =  sfConfig::get('propel_behavior_sortableFk_'.get_class($object).'_f_key', 'object_id');
    $fKeyColumnPhpName = call_user_func(array($peerClass, 'translateFieldName'), $fKeyColumnName, BasePeer::TYPE_FIELDNAME, BasePeer::TYPE_COLNAME);
    if(!$con) $con = Propel::getConnection(constant("$peerClass::DATABASE_NAME"));
  ...
  {
    return $object->getByName(sfConfig::get('propel_behavior_sortableFk_'.get_class($object).'_f_key', 'object_id'), BasePeer::TYPE_FIELDNAME);
  }
  ...
  {
    return self::retrieveByPosition(get_class($object).'Peer', self::getPosition($object) + 1, self::getFKey($object));
  }
  ...
  {
    return self::getPosition($object) == self::getMinPosition(get_class($object).'Peer', self::getFKey($object));
  ...
  {
    return self::getPosition($object) == self::getMaxPosition(get_class($object).'Peer', self::getFKey($object));
  }
  ...
  public function moveToPosition($object, $newPosition, $con = null)
  {
    ...
    try
    {
      ...
      self::setPosition($object, self::getMaxPosition($peerClass, self::getFKey($object)) + 1);
      $object->save();
      
      // Shift the objects between the old and the new position
      $query = sprintf('UPDATE %s SET %s = %s %s 1 WHERE (%s BETWEEN ? AND ?) AND (%s = ?)',
        constant("$peerClass::TABLE_NAME"),
        $rankColumnName,
        $rankColumnName,
  ...
  public function moveToBottom($object, $con = null)
  {
    ...
    return self::moveToPosition($object, self::getMaxPosition($peerClass, self::getFKey($object), $con), $con);
  }
