...
class twitterAnnounce extends BaseAnnounce{
  ...
  public function announceMm(Mm $mm){
    file_put_contents(sfConfig::get('sf_log_dir') . '/announces.log', "twitter mm" . $mm->getId() . " \n", FILE_APPEND);    
    $this->pr();
  }

  public function announceSerial(Serial $serial){
    file_put_contents(sfConfig::get('sf_log_dir') . '/announces.log', "twitter serial" . $serial->getId() . " \n", FILE_APPEND);    
    $this->pr();
  }
