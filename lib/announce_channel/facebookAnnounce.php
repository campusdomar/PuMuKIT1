...
class facebookAnnounce extends BaseAnnounce{
  ...
  public function announceMm(Mm $mm){
    file_put_contents(sfConfig::get('sf_log_dir') . '/announces.log', "facebook mm" . $mm->getId() . " \n", FILE_APPEND);    
    $this->pr();
  }

  public function announceSerial(Serial $serial){
    file_put_contents(sfConfig::get('sf_log_dir') . '/announces.log', "facebook serial" . $serial->getId() . " \n", FILE_APPEND);    
    $this->pr();
  }
