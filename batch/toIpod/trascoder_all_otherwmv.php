...
//OTROS
$c->add(FilePeer::PERFIL_ID, array(3,5,8,18,12), Criteria::IN);
...
foreach($files as $f){
  ...
  $movie = new ffmpeg_movie($f->getFile());
  echo "Resolution: ", $movie->getFrameWidth(), "x", $movie->getFrameHeight(), "\n";
  echo "Aspect: ",($movie->getFrameWidth() / $movie->getFrameHeight()), "\n";


  $trans = new Transcoding();
  if(($movie->getFrameWidth() / $movie->getFrameHeight()) < 1.5){
    //echo "------------------------>>>>>>\n4x3\n";
    $trans->setPerfilId(21); //4_3
  }else{
    //echo "------------------------>>>>>>\n16x9\n";
    $trans->setPerfilId(20); //16_9
  }
